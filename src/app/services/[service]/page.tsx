...
export default function ServicePage({ params }: { params: { service: string } }) {
  ...
  const formatName = FORMAT_TABS.find(
    (tab) => tab.toLowerCase() === serviceSlug.toLowerCase()
  ) || "Loading";
  ...
  return (
    ...
    >
      <ServiceGallery formatName={formatName} />
    </Suspense>
  );
}

function ServiceGallery({ formatName }: { formatName: string | undefined }) {
  const searchParams = useSearchParams();
  const initialIdx = parseInt(searchParams.get("videoIdx") || "0", 10);

  const activeVideos = formatName && formatName !== "Loading" ? FORMATS_DATA[formatName] : [];
  ...
  if (!formatName || formatName === "Loading" || !activeVideos || activeVideos.length === 0) {
    return (
      <div className="min-h-screen bg-[#050508] flex items-center justify-center flex-col gap-6 text-white">
        ...
        <Link href="/" className="px-6 py-3 bg-[#6EE7FF] text-[#050508] font-bold rounded-full">
          Return Home
        </Link>
      </div>
    );
...
function VideoCard({ video, isThumbnail = false, isActive = false }: {
  video: VideoData;
  isThumbnail?: boolean;
  isActive?: boolean;
}) {
